...
class RLINE(object):
    ...
    def buildGraph(self):
        ...
        embedding = tf.placeholder(tf.float32,[opts.num_nodes, opts.emb_dim])
        ...
        R_matrices = tf.Variable(tf.random_uniform([opts.num_relations, opts.relation_matrix_dim, opts.emb_dim], -0.5, 0.5), name="r_matrices")
    ...
    def trainFirst(self):
        ...
        for _ in xrange(opts.max_epoch):
            if random.random() <= 1.0:
                ...
                self._session.run(self.optimizer, feed_dict={self.U_batch: train_U_labeled, self.V_batch: train_V_labeled, self.relation_labels: rel_labels, self.embedding: self._line_embs})
            else:
            ...
            if n_iter % opts.disp_freq == 0:
                ...
                loss_labeled = self._session.run(self.loss, feed_dict={self.U_batch: train_U_labeled, self.V_batch: train_V_labeled, self.relation_labels: rel_labels, self.embedding: self._line_embs})                
                print('step %d, loss_label =%f ' % (n_iter, loss_labeled))
